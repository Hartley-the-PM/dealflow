...
export default function PackageViewerPage() {
  const hydrated = useHydration();
  const params = useParams();
  const token = params.token as string;

  const getPackageByToken = useDocumentStore((s) => s.getPackageByToken);
  const addPackageView = useDocumentStore((s) => s.addPackageView);

  const pkg = getPackageByToken(token);

  // Log view on mount
  useEffect(() => {
    if (!hydrated || !pkg) return;
    if (pkg.status === 'Revoked' || isPast(new Date(pkg.expiryDate))) return;

    addPackageView(pkg.id, {
      id: uuidv4(),
      viewedAt: new Date().toISOString(),
      ipAddress: '127.0.0.1', // simulated
      userAgent: typeof navigator !== 'undefined' ? navigator.userAgent : 'Unknown',
      email: pkg.recipientEmail || '',
    });
    ...
    return (
      <Box sx={{ maxWidth: 600, mx: 'auto', mt: 10, textAlign: 'center' }}>
        <Card variant="outlined">
          <CardContent sx={{ py: 6 }}>
            <BlockIcon sx={{ fontSize: 48, color: '#DC2626', mb: 2 }} />
            <Typography variant="h5" fontWeight={600} sx={{ mb: 1 }}>
              Package Not Found
            </Typography>
            <Typography color="text.secondary">
              This document package link is invalid or no longer exists.
            </Typography>
          </CardContent>
        </Card>
      </Box>
  ...
  // Expired or revoked
  if (pkg.status === 'Revoked' || isPast(new Date(pkg.expiryDate))) {
    return (
      <Box sx={{ maxWidth: 600, mx: 'auto', mt: 10, textAlign: 'center' }}>
        <Card variant="outlined">
          <CardContent sx={{ py: 6 }}>
            ...
            <Typography color="text.secondary">
              This document package is no longer available. Please contact the sender for an updated link.
            </Typography>
          </CardContent>
        </Card>
      </Box>
    ...
    <Box sx={{ maxWidth: 800, mx: 'auto', mt: 4, px: 2 }}>
      ...
      <Box sx={{ mb: 4, textAlign: 'center' }}>
        <Box sx={{ display: 'flex', justifyContent: 'center', mb: 2 }}>
          <svg width="140" height="45" viewBox="0 0 251 80" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M69 23.3885C69 29.6782 63.967 34.777 57.7585 34.777C51.5499 34.777 46.5169 29.6782 46.5169 23.3885C46.5169 17.0988 51.5499 12 57.7585 12C63.967 12 69 17.0988 69 23.3885Z" fill="url(#vl0p)"/>
            <path d="M22.2415 12C28.4501 12 33.4831 17.0988 33.4831 23.3885C33.4831 24.0405 33.4288 24.6796 33.3249 25.3016C31.5414 37.1393 30.2383 40.4851 44.5437 45.9823C48.9834 47.9812 51.4498 52.2896 51.4498 56.4768C51.4498 62.4737 46.5169 68.4705 38.6243 67.9708C33.1981 66.9713 29.2517 63.4731 28.7585 56.9765C28.7585 55.6443 28.9415 54.3272 29.2504 53.0313C31.9809 41.5732 29.6427 37.8139 19.0155 34.3009C14.3792 32.8963 11 28.5423 11 23.3885C11 17.0988 16.033 12 22.2415 12Z" fill="url(#vl1p)"/>
            ...
            <path d="M132.054 46.5794H123.206C118.95 46.5794 116.822 48.1661 116.822 51.3394C116.822 52.7581 117.382 53.8968 118.502 54.7554C119.622 55.6141 121.134 56.0434 123.038 56.0434C125.838 56.0434 128.04 55.3154 129.646 53.8594C131.251 52.3661 132.054 50.4061 132.054 47.9794V46.5794Z" fill="#1F1F1F"/>
            <path d="M80 20.4277H85.152L97.136 53.8597L109.064 20.4277H114.104L99.992 59.6277H94.112L80 20.4277Z" fill="#1F1F1F"/>
            <defs>
              <linearGradient id="vl0p" x1="14.9658" y1="12" x2="79.3605" y2="29.1906" gradientUnits="userSpaceOnUse">
                <stop stopColor="#FAEC8C"/><stop offset="1" stopColor="#FD6437"/>
              </linearGradient>
              <linearGradient id="vl1p" x1="14.9658" y1="12" x2="79.3605" y2="29.1906" gradientUnits="userSpaceOnUse">
                <stop stopColor="#FAEC8C"/><stop offset="1" stopColor="#FD6437"/>
              </linearGradient>
            </defs>
          </svg>
        </Box>
        <Typography variant="h5" fontWeight={700} sx={{ mb: 0.5, color: '#111827' }}>
          {pkg.name}
        </Typography>
        <Typography variant="body2" color="text.secondary">
          {pkg.documents.length} document{pkg.documents.length !== 1 ? 's' : ''} · Valid until {format(new Date(pkg.expiryDate), 'MMMM d, yyyy')}
        </Typography>
      </Box>
      ...
      <Card variant="outlined" sx={{ mb: 3 }}>
        <CardContent>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
            ...
            <Button variant="contained" size="small" startIcon={<FolderZipIcon />} sx={{ bgcolor: '#E97A2B', '&:hover': { bgcolor: '#D4691F' } }}>
              Download All
            </Button>
          </Box>
          ...
          <TableContainer>
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: 600, fontSize: '0.8rem' }}>Document</TableCell>
                  <TableCell sx={{ fontWeight: 600, fontSize: '0.8rem' }}>Type</TableCell>
                  <TableCell sx={{ fontWeight: 600, fontSize: '0.8rem' }}>Date</TableCell>
                  <TableCell align="right" sx={{ fontWeight: 600, fontSize: '0.8rem' }}>Download</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                  ...
                  <TableRow key={doc.id} hover>
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <DescriptionIcon fontSize="small" sx={{ color: '#E97A2B' }} />
                        <Box>
                    ...
                    <TableCell>
                      <Chip label={doc.type} size="small" sx={{ fontSize: '0.65rem', bgcolor: '#FFF7ED', color: '#E97A2B', fontWeight: 500 }} />
                    </TableCell>
                    <TableCell sx={{ fontSize: '0.85rem' }}>
                      {doc.date ? format(new Date(doc.date), 'MMM d, yyyy') : '\u2014'}
                    </TableCell>
                    <TableCell align="right">
                      <IconButton size="small" sx={{ color: '#E97A2B' }}>
